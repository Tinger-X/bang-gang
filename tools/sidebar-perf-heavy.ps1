...
#
# The fixture is 6 messages, one reply ~14k chars, matching the real history., a code block and a table each, written into
...
function New-AssistantText([int]$n, [int]$repeat) {
...
function Write-Fixture {
    ...
    for ($i = 1; $i -le 3; $i++) {
        ...
        $msgs += New-Body 'assistant' (New-AssistantText $i 28)
    }
    # ChatFile envelope, not a bare Conversation -- see the note in markdown-render.ps1.
    $file = @{
        Version = 1
        Conversation = @{
            Id = $script:ConvId; Title = 'sidebar-perf-heavy'; CreatedAt = $now; UpdatedAt = $now
            Messages = $msgs
        }
    }
...
try {
    Invoke-BBProbe {
        ...
        if ($rows.Count -ne 6) {
            Write-Output ('  FAIL -- ' + $rows.Count + ' rows, expected 6; the timings below would be for a lighter screen than intended')
            $script:fail++
        }
        else { Write-Output '  OK   6 rows, heavy enough to be worth measuring' }
        ...
        # This fixture's 14k-char reply fills the band by itself, so its fresh-width frames
        # sit at the top of that range -- that is the price of LIVE re-wrap, paid per frame
        # instead of as one big hitch at settle.
...
Write-BBDone 'sidebar-perf-heavy'
