...
/// 三条交互约定：
///
/// 1. <b>回车提交</b>、<b>Esc 取消</b>。<see cref="Control.ProcessCmdKey"/> 那一条不能省：
///    Esc 在 WinForms 里是「对话框键」，单行 EDIT 根本不会把它交出来当 KeyDown
///    （同一个坑见 <see cref="InputField"/>、<see cref="TextArea"/>），只挂 KeyDown 的话
///    按 Esc 会一路冒泡到主窗口的快捷键处理里去。
/// 2. <b>点走（失去焦点）等于提交</b>，不是取消 —— 用户点到别处通常是「改完了」。
///    取消有明确的出口（Esc），而「点空白处丢掉刚敲的字」没有第二次机会。
...
/// </summary>
internal sealed class TitleEditor : Control, IThemed
{
